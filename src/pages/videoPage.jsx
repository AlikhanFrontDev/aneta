import * as React from "react";
import PropTypes from "prop-types";
import Tabs from "@mui/material/Tabs";
import Tab from "@mui/material/Tab";
import Typography from "@mui/material/Typography";
import Box from "@mui/material/Box";
import styled from "styled-components";

function VideoPage(props) {
  const { children, value, index, ...other } = props;

  return (
    <div
      role="tabpanel"
      hidden={value !== index}
      id={`simple-tabpanel-${index}`}
      aria-labelledby={`simple-tab-${index}`}
      {...other}
    >
      {value === index && (
        <Box sx={{ p: 13 }}>
          <Typography>{children}</Typography>
        </Box>
      )}
    </div>
  );
}

VideoPage.propTypes = {
  children: PropTypes.node,
  index: PropTypes.number.isRequired,
  value: PropTypes.number.isRequired,
};

function a11yProps(index) {
  return {
    id: `simple-tab-${index}`,
    "aria-controls": `simple-tabpanel-${index}`,
  };
}

export default function BasicTabs() {
  const [value, setValue] = React.useState(0);

  const handleChange = (event, newValue) => {
    setValue(newValue);
  };

  return (
    <Container>
      <Box sx={{ width: "100%" }}>
        <Box sx={{ borderBottom: 1, color: "red"}}>
          <Tabs
            sx={{display: "flex", flexDirection: "column", color: "red"}}
            value={value}
            onChange={handleChange}
            aria-label="basic tabs example"
            className="tabs"
            vertical= {true}
          >
            <Tab label="Item One" {...a11yProps(0)} />
            <Tab label="Item Two" {...a11yProps(1)} />
            <Tab label="Item Three" {...a11yProps(2)} />
            <Tab label="Item Three" {...a11yProps(3)} />
            <Tab label="Item Three" {...a11yProps(4)} />
            <Tab label="Item Three" {...a11yProps(5)} />
            <Tab label="Item Three" {...a11yProps(6)} />
            <Tab label="Item Three" {...a11yProps(8)} />
            <Tab label="Item Three" {...a11yProps(9)} />
            <Tab label="Item Three" {...a11yProps(10)} />
            <Tab label="Item Three" {...a11yProps(11)} />
            <Tab label="Item Three" {...a11yProps(12)} />
            <Tab label="Item Three" {...a11yProps(13)} />
          </Tabs>
        </Box>
        <VideoPage value={value} index={0}>
          Item One
        </VideoPage>
        <VideoPage value={value} index={1}>
          Item Two
        </VideoPage>
        <VideoPage value={value} index={2}>
          Item Three
        </VideoPage>
        <VideoPage value={value} index={3}>
          Item Threeasdasd
        </VideoPage>
        <VideoPage value={value} index={4}>
          Item Threeasdasd
        </VideoPage>
        <VideoPage value={value} index={5}>
          Item Threeasdasd
        </VideoPage>
        <VideoPage value={value} index={6}>
          Item Threeasdasd
        </VideoPage>
        <VideoPage value={value} index={7}>
          Item Threeasdasd
        </VideoPage>
        <VideoPage value={value} index={8}>
          Item Three
        </VideoPage>
        <VideoPage value={value} index={9}>
          Item Three
        </VideoPage>
      </Box>
    </Container>
  );
}
const Container = styled.div`
  color: red;
  .tabs{
    color: red;
  }
`;
